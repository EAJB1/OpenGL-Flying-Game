...
void Model::loadModel(string const& path)
{
    ...
    // Store the full directory path of the model
...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
    ...
    // Cycle through all the meshe's vertices
    for (unsigned int i = 0; i < mesh->mNumVertices; i++)
    {
        ...
        // Texture coordinates (only if there are any)
    ...
    // Cycle through all mesh faces and set vertex indices.
...
vector<Texture> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName)
{
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        {
            // Check if a texture with the same filepath has already been loaded.
